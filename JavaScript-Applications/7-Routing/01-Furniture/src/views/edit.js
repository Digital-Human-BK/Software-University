...
<div class="row space-top">
  ...
  ${until(itemPromise, html`<h2>Loading Item &hellip;</h2>`)}
</div>`;
...
export function editPage(ctx) {
  ...
  async function onSubmit(ev) {
    ...
    const data = formData.reduce((acc, [k, v]) => Object.assign(acc, { [k]: v.trim() }), {});
    ...
    try {
      if (missing.length > 0) {
        ...
        throw {
          err: new Error('Please fill all mandatory fields!'),
          errors
        }
      }
      ...
      const message = error.message || error.err.message;
