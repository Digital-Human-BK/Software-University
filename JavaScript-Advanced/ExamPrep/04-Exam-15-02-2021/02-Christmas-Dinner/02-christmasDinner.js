class ChristmasDinner {
  ...
  recipes(recipe) {
    for (const item of recipe.productsList) {
      if (this.products.includes(item) === false) {
  ...
  inviteGuests(name, dish) {
    let dishIsPresent = this.dishes.some(d => d.recipeName === dish);
    if (dishIsPresent == false) {
  ...
  showAttendance() {
    ...
    Object.keys(this.guests).forEach(g => {
      ...
      let products = this.dishes.find(item => item.recipeName === dish).productsList.join(', ');
